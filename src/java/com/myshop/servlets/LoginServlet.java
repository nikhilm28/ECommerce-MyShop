...
public class LoginServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        /*    Login login = new Login();
            ...
            try
            {
                String authenticateuser = logindao.validateUser(login);
                
                if(authenticateuser.equals("Admin_Role"))
                {
                ...
                else if(authenticateuser.equals("Normal"))
                {
                    System.out.println("Normal");
                    HttpSession session = request.getSession();
                     session.setAttribute("current-user", login);
                    session.setAttribute("normal", phone);
                ...
                {
                    HttpSession session = request.getSession();
                    session.setAttribute("alert_message", "User not found!!!");
                    response.sendRedirect("login.jsp");
                }
            }
            catch(IOException e)
